...
The middleware components are added `Configure` when configuring the application in `Startup.cs`. The following code sets up middleware for all available content.
...
This call is equivalent to setting the entire web application and adding the following middleware (in the specified order). Please note that the integrated middleware handles a lot of different content types, specified in the process order.
