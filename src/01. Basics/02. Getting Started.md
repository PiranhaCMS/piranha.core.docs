...
Enables runtime compilation for Razor `.cshtml` files. This is really useful for development environments so that you don't have to restart the application every time you update a view or page. A good idea is to check the runime environment using the `IWebHostEnvironment`.

~~~ csharp
    services.AddPiranha(options =>
    {
        options.AddRazorRuntimeCompilation = Env.IsDevelopment();
    });
~~~
